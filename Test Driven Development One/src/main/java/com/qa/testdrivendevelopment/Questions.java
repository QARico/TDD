...
public class Questions {
	...
	{
		String a = new StringBuilder(sandwich).reverse().toString();
		return a;
	}
	...
	{
		int index = sentence.indexOf("am ");
		int count = 0;
		while (index != -1)
		{
			count++;
			sentence = sentence.substring(index + 1);
			index = sentence.indexOf("am ");
		}
    	return count;
	}
	...
	{
		boolean checker;
		if (word.charAt(index) == letter && index < word.length())
		{
			checker = true;
		}
		else
